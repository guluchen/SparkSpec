 ...
 */
object RandomizedVertexColoring {
  ...
  def run (graph: GraphRDD[Color, _], beta: Double, maxNumColors: Int): GraphRDD[Color, _] = {
    ...
    val distGraph = GraphRDD(mapVertices(graph.vertexRDD) {
      v => (v.attr, initColorDist, true, seed + v.id)
    }, graph.edgeRDD)
    ...
    def vprog (v: Vertex[Palette], active: Boolean): Palette = {
      ...
      val new_color = if (active) sampleColor(new_dist, Random.nextDouble) else color

      (new_color, new_dist, active, Random.nextLong)
    }
...
object VertexColoringExample {
  ...
  def run = {
    ...
    */
    ...
    val result = RandomizedVertexColoring.run(graph, beta, maxNumColors)
